...
def analyze_property():
    ...
    try:
        ...
        monthly_mortgage_payment = loan_amount * (monthly_rate * (1 + monthly_rate) ** total_payments) / (
                    (1 + monthly_rate) ** total_payments - 1)

        # calculations
        address_title = analysis_params["Address"] + ', ' + analysis_params['City'] + ', ' + analysis_params[
            'State'] + ', ' + analysis_params['Zip Code']
        ...
        return jsonify({'error': f'An unexpected error occurred while building your dashboard: {str(e)}. Please try '
                                 f'another property file or try again later.'}), 500
